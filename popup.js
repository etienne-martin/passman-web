// Copyright (c) 2016 Etienne Martin. All rights reserved.

(function() {
	
	var passwordRequirements = "A!"; // lowercase, uppercase, digit and special char.

	var domainLabel = document.getElementsByTagName("input")[0];
	var mainInput = document.getElementsByTagName("input")[1];
	var resultInput = document.getElementsByTagName("input")[2];
	
	var hostname = "";
	
	domainLabel.addEventListener('input', function(e){
				
		hostname = e.target.value.toLowerCase().split(".").reverse();
		hostname = hostname[1]+"."+hostname[0]; // Remove sub domains
		
	}, false);
	
	mainInput.addEventListener('input', function(e){
			
		var hash = CryptoJS.SHA3(e.target.value+":"+hostname, {outputLength: 128});
		var password = passwordRequirements+hash;	
			password = e.target.value.length===0?'':password.substr(0, e.target.value.length*4);
